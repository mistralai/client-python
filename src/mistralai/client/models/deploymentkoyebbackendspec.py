"""Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT."""
# @generated-id: 756d9f222217

from __future__ import annotations
from mistralai.client.types import (
    BaseModel,
    Nullable,
    OptionalNullable,
    UNSET,
    UNSET_SENTINEL,
)
from mistralai.client.utils import validate_const
import pydantic
from pydantic import model_serializer
from pydantic.functional_validators import AfterValidator
from typing import Literal
from typing_extensions import Annotated, NotRequired, TypedDict


class DeploymentKoyebBackendSpecTypedDict(TypedDict):
    r"""Worker configuration for the Koyeb backend."""

    type: Literal["koyeb"]
    build_directory: NotRequired[Nullable[str]]
    r"""Docker build context, as a path in the repo. Defaults to the repo root."""
    dockerfile_path: NotRequired[Nullable[str]]
    r"""Path to the Dockerfile, relative to 'build_directory'. Defaults to 'Dockerfile'."""


class DeploymentKoyebBackendSpec(BaseModel):
    r"""Worker configuration for the Koyeb backend."""

    type: Annotated[
        Annotated[Literal["koyeb"], AfterValidator(validate_const("koyeb"))],
        pydantic.Field(alias="type"),
    ] = "koyeb"

    build_directory: OptionalNullable[str] = UNSET
    r"""Docker build context, as a path in the repo. Defaults to the repo root."""

    dockerfile_path: OptionalNullable[str] = UNSET
    r"""Path to the Dockerfile, relative to 'build_directory'. Defaults to 'Dockerfile'."""

    @model_serializer(mode="wrap")
    def serialize_model(self, handler):
        optional_fields = set(["build_directory", "dockerfile_path"])
        nullable_fields = set(["build_directory", "dockerfile_path"])
        serialized = handler(self)
        m = {}

        for n, f in type(self).model_fields.items():
            k = f.alias or n
            val = serialized.get(k, serialized.get(n))
            is_nullable_and_explicitly_set = (
                k in nullable_fields
                and (self.__pydantic_fields_set__.intersection({n}))  # pylint: disable=no-member
            )

            if val != UNSET_SENTINEL:
                if (
                    val is not None
                    or k not in optional_fields
                    or is_nullable_and_explicitly_set
                ):
                    m[k] = val

        return m


try:
    DeploymentKoyebBackendSpec.model_rebuild()
except NameError:
    pass
